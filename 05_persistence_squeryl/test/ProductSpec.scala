...
class ProductSpec extends Specification with DatabaseHelpers {
  "Products" should {
    ...
    "be queryable on warehouse" in WarehousesStockItemsAndProducts {
      Product.productsInWarehouse(forbiddenPlanet) must
        containTheSameElementsAs(List(opticNerve, sandman, starman))
    }

    "be queryable on warehouse by product name" in WarehousesStockItemsAndProducts {
      Product.productsInWarehouseByName(starman.name, gosh) must
        containTheSameElementsAs(List(starman))
    }
